...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			...
			'language_id': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_categories_description WHERE language_id IN " + self.list_to_in_condition(category_ids)
			},
			# 'cate_imgs': {
			# 	'type': 'select',
	...
	def convert_category_export(self, category, categories_ext):
		...
		# img = get_row_from_list_by_field(categories_ext['data']['categories_image'], 'categories_id', category['categories_id'])
		...
		# self.log(categories_ext['data'], 'abc')
		category_description = get_row_from_list_by_field(categories_ext['data']['categories_name'], 'categories_id', category['categories_id'])
		...
		# self.log(response_success(category_data), 'test')
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'products_description': {
				'type': "select",
				'query': "SELECT * FROM  _DBPRF_products_description WHERE products_id IN  " + product_id_con  # + " ORDER BY disp_order",
			},
			'products_image': {
				'type': "select",
				'query': "SELECT * FROM  _DBPRF_products WHERE products_id IN  " + product_id_con
			},
	...
	def convert_product_export(self, product, products_ext):
		...
		for product_description in products_ext['data']['products_description']:
			if product_description['products_id'] == product['products_id']:
				product_data['name'] = product_description['products_name']
				product_data['description'] = product_description['products_description']
		# product_data['name'] = html.unescape(products_ext['data']['products_description'][1]['products_name'])
		# product_data['description'] = html.unescape(products_ext['data']['products_description'][1]['products_description'])
		...
		url_product_image = self._notice['src']['cart_url'] + '/images/'
		...
		product_images = get_list_from_list_by_field(products_ext_data['images'], 'products_image', product['products_image'])
		if product_images:
			if check_thumbnail:
				for product_image in product_images:
					image_data = get_row_from_list_by_field(products_ext_data['images'], 'products_image', product_image['products_image'])
	...
	def get_orders_ext_export(self, orders):
		...
		orders_ext_queries = {
			...
			'orders_payment': {
				'type': 'select',
				'query': "SELECT payment_method FROM  _DBPRF_orders WHERE orders_id  IN " + self.list_to_in_condition(order_ids)
			},
			...
			'currencies': {
				'type': 'select',
				'query': "SELECT * FROM currencies WHERE currencies_id IN " + self.list_to_in_condition(order_ids)
			},
			'countries': {
				'type': 'select',
	...
	def convert_order_export(self, order, orders_ext):
		...
		#
		# customer_state = get_row_from_list_by_field(orders_ext['data']['zones'], 'code', order['delivery_state'])
		# if customer_state:
		...
		# billing_name = self.get_name_from_string(order['billing_name'] + ' ' + order['bill_lname'])
		...
		# delivery_name = self.get_name_from_string(order['delivery_name'])# + ' ' + order['ship_lname'])
		...
		payments = get_row_from_list_by_field(orders_ext['data']['orders_payment'], 'payment_method', order['payment_method'])
